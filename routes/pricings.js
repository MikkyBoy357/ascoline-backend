...
router.get(
  "/",
  authorizeJwt,
  verifyAccount([{ name: "pricing", action: "read" }]),
  async (req, res) => {
    let count;
    ...
    try {
      count = await Pricing.countDocuments(filter);
      const pricings = await Pricing.find(filter)
        .populate("transportType unit typeColis", "label")
        .limit(limit)
        .skip((page - 1) * limit);
      res.status(200).json({
        pricings,
  ...
  async (req, res) => {
    try {
      ...
      if (!pricing) {
        return res
          .status(404)
          .json({ message: `Cannot find any Pricing with ID ${id}` });
      }
...
router.post(
  "/",
  authorizeJwt,
  verifyAccount([{ name: "pricing", action: "create" }]),
  async (req, res) => {
    try {
...
router.put(
  "/:id",
  authorizeJwt,
  verifyAccount([{ name: "pricing", action: "update" }]),
  async (req, res) => {
    try {
      const { id } = req.params;
...
router.delete(
  "/:id",
  authorizeJwt,
  verifyAccount([{ name: "pricing", action: "delete" }]),
  async (req, res) => {
    try {
      const { id } = req.params;
